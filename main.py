...
def formulario(update: Update, context: CallbackContext) -> None:
    ...
    update.message.reply_text(
        'Escriba "{}" para prometer total responsabilidad y honestidad al responder este formulario'.format(FRASE_AUTORIZACION)
    )
...
def button(update: Update, context: CallbackContext) -> None:
    ...
    if len(usuarios[uid][0]) == 1 and len(usuarios[uid][1]) == 1:
        ...
        if usuarios[uid][0][0] == '0':
            ...
            query.edit_message_text("Area a visitar", reply_markup=reply_markup)
            usuarios[uid][1].append(1)
        #opcion cuando son personal
        elif usuarios[uid][0][0] == '1':
            reply_markup = InlineKeyboardMarkup(KB_PERSONAL)
            query.edit_message_text("Tipo de personal", reply_markup=reply_markup)
            usuarios[uid][1].append(1)
            
        #opcion cuando son estudiantes
        elif usuarios[uid][0][0] == '2':
            reply_markup = InlineKeyboardMarkup(KB_CARRERAS)
            query.edit_message_text("Carrera del estudiante", reply_markup=reply_markup)
            usuarios[uid][1].append(1)
    else:
        #se solicita matricula o id
        if usuarios[uid][0][0] != '0' and len(usuarios[uid][1]) < 3:
            query.edit_message_text(text=f"Ingrese su matricula o numero de empleado: ")
            usuarios[uid][1].append(1)
            ...
            query.edit_message_text(PREGUNTAS[len(usuarios[uid][0])-len(usuarios[uid][1])], reply_markup=reply_markup)
    ...
    try:
        if  len(usuarios[uid][0]) == 0 and len(usuarios[uid][1]) == 1:
            ...
            if(respuestaUsuario == FRASE_AUTORIZACION):
                ...
                update.message.reply_text('¿Tipo de usuario?', reply_markup=reply_markup)
                return
            else:
                update.message.reply_text(
...
def main() -> None:
    ...
    dispatcher.add_handler(MessageHandler(Filters.text & ~Filters.command, answer_command))
